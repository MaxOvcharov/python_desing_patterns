...
class CheckersBoard(AbstractBoard):
    ...
    def populate_board(self):  # Thanks to Doug Hellmann for the idea!
        ...
        self.board = [
            list(itertools.islice(itertools.cycle(squares), 0, len(rows))) for squares in rows
        ]
...
class ChessBoard(AbstractBoard):
    ...
    def populate_board(self):
        figures = (((0, 7), ROOK), ((1, 6), KNIGHT), ((2, 5), BISHOP), ((3,), QUEEN), ((4,), KING))

        for row, color in ((0, BLACK), (7, WHITE)):
            for columns, kind in figures:
